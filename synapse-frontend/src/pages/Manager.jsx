...
function Manager() {
  ...
  const carregarDashboards = async () => {
    try {
      const response = await axios.get(`${ìp}:2399/api/synapse-ti/api/dashboards/usuario/${usuarioId}`);
  ...
  const handleAdicionar = async (e) => {
    ...
    try {
      await axios.post(`${ìp}:2399/api/synapse-ti/api/dashboards/usuario/${usuarioId}`, {
  ...
  const handleDelete = async (id) => {
    if(window.confirm("Tem certeza que deseja excluir este dashboard?")) {
      try {
        await axios.delete(`${ìp}:2399/api/synapse-ti/api/dashboards/${id}`);
        carregarDashboards(); 
      } catch (error) {
  ...
  const salvarEdicao = async (id) => {
    try {
      await axios.put(`${ìp}:2399/api/synapse-ti/api/dashboards/${id}/tempo`, {
